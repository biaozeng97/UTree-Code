...
setwd("C:/Unfolding Tree Model/ERSUTree")
...
Generate.sample<-function(condition, fixed_objects = NULL) {
  ...
  beta2 <- runif(n = 2*I, min = -2, max = 2) # item difficulty in the negative direction
  ...
  data.gen <- function(alpha1,delta1,tau1,alpha2,beta2,
                       N,I,Sigma,Theta){
    ...
    for (n in 1:N){
      ...
      for (i in 1:I) {
        ...
        # negative direction
        p2.0 <- 1/(1+exp(alpha2[i]*(eta[n]-beta2[i])))
        p2.1 <- exp(alpha2[i]*(eta[n]-beta2[i]))/(1+exp(alpha2[i]*(eta[n]-beta2[i])))
        
        ## positive direction
        p3.0 <- 1/(1+exp(alpha2[I+i]*(eta[n]-beta2[I+i])))
        p3.1 <- exp(alpha2[I+i]*(eta[n]-beta2[I+i]))/(1+exp(alpha2[I+i]*(eta[n]-beta2[I+i])))
        
        
        
        ## Probability for observed categories
        p <- c(p1.0*p2.1, # strongly disagree
               p1.0*p2.0, # disagree, UTree set the disagree as 1,  strongly disagree as 0
               p1.1*p3.0, # agree
    ...
    ## Generate data 
    data.4 <- data.gen(alpha1,delta1,tau1,alpha2,beta2,
                       N,I,Sigma) # dataset for the GGUM model in node 1, 2 and 3
    ...
    dat <- list(data.4,data.2,data.6, alpha1.raw,delta1.raw,tau1.raw,
                alpha2.raw,beta2.raw,theta.raw,eta.raw)
    dat
  }
...
Analyse<-function(condition,dat,fixed_objects=NULL){
  ...
  beta2.raw <- as.vector(dat[[8]])
  theta.raw <- as.vector(dat[[9]])
  eta.raw <- as.vector(dat[[10]])
  ...
  # report item parameter and transform d to beta in 2PL models
  ...
  if(isTRUE(max(abs(para.ORDUTree_1[,2:3]))>=10) | isTRUE(max(abs(para.ORDUTree_2[,3:5]))>=10) | isTRUE(max(abs(para.ERSUTree[,3:6]))>=10)){stop("Delta estimates reached the limit")}
  ...
  sign.ORDUTree_2.delta1 <- sign(cor(para.ORDUTree_2[1:I,3], delta1.raw))
  sign.ORDUTree_2.delta2 <- sign(cor(para.ORDUTree_2[(I+1):(3*I),4], beta2.raw))
  sign.ERSUTree.delta1 <- sign(cor(para.ERSUTree[1:I,3],delta1.raw))
  ...
  sign.ERS.theta <- sign(cor(theta.ERS[,1], theta.raw))
  theta.esti.ERS <- theta.ERS[,1] *sign.ERS.theta
  theta.combine.ERS  <- data.frame(cbind(theta.esti.ERS, theta.raw))
  sign.ERS.eta <- sign(cor(theta.ERS[,2], eta.raw))
  eta.esti.ERS <- theta.ERS[,2]*sign.ERS.eta
  eta.combine.ERS <- data.frame(cbind(eta.esti.ERS,eta.raw))
  
  sign.ORD_1.theta <- sign(cor(theta.ORD_1[,1], theta.raw))
  theta.esti.ORD_1 <- theta.ORD_1[,1] *sign.ORD_1.theta
  theta.combine.ORD_1  <- data.frame(cbind(theta.esti.ORD_1, theta.raw))
  
  sign.ORD_2.theta1 <- sign(cor(theta.ORD_2[,1], theta.raw))
  theta1.esti.ORD_2 <- theta.ORD_2[,1] *sign.ORD_2.theta1
  theta1.combine.ORD_2  <- data.frame(cbind(theta1.esti.ORD_2, theta.raw))
  sign.ORD_2.theta2 <- sign(cor(theta.ORD_2[,2], eta.raw))
  theta2.esti.ORD_2 <- theta.ORD_2[,2]*sign.ORD_2.theta2
  theta2.combine.ORD_2 <- data.frame(cbind(theta2.esti.ORD_2,eta.raw))
  
  sign.ERSUTree.theta <- sign(cor(theta.ERSUTree[,1], theta.raw))
  theta.esti.ERSUTree <- theta.ERSUTree[,1] *sign.ERSUTree.theta
  theta.combine.ERSUTree  <- data.frame(cbind(theta.esti.ERSUTree ,theta.raw))
  sign.ERSUTree.eta <- sign(cor(theta.ERSUTree[,2], eta.raw))
  eta.esti.ERSUTree <- theta.ERSUTree[,2]*sign.ERSUTree.eta
  eta.combine.ERSUTree <- data.frame(cbind(eta.esti.ERSUTree,eta.raw))
  
  sign.ORDUTree_1.theta <- sign(cor(theta.ORDUTree_1[,1], theta.raw))
  theta.esti.ORDUTree_1 <- theta.ORDUTree_1[,1] *sign.ORDUTree_1.theta
  theta.combine.ORDUTree_1  <- data.frame(cbind(theta.esti.ORDUTree_1 ,theta.raw))
  
  sign.ORDUTree_2.theta1 <- sign(cor(theta.ORDUTree_2[,1], theta.raw))
  theta1.esti.ORDUTree_2 <- theta.ORDUTree_2[,1] *sign.ORDUTree_2.theta1
  theta1.combine.ORDUTree_2  <- data.frame(cbind(theta1.esti.ORDUTree_2 ,theta.raw))
  sign.ORDUTree_2.theta2 <- sign(cor(theta.ORDUTree_2[,2], eta.raw))
  theta2.esti.ORDUTree_2 <- theta.ORDUTree_2[,2]*sign.ORDUTree_2.theta2
  theta2.combine.ORDUTree_2 <- data.frame(cbind(theta2.esti.ORDUTree_2,eta.raw))
  ...
  ret<-c(
    ...
    Abis.delta1.ERSUTree <- mean(abs(para.ERSUTree[1:I,3]*sign.ERSUTree.delta1-delta1.raw)),
    Abis.beta2.ERSUTree <- mean(abs(para.ERSUTree[(I+1):(3*I),9]*sign.ERSUTree.beta2-beta2.raw)),
    Abis.tau1.ERSUTree <- mean(abs(para.ERSUTree[1:I,5]-tau1.raw)),
    ...
    Abis.delta1.ORDUTree_2 <- mean(abs(para.ORDUTree_2[1:I,3]*sign.ORDUTree_2.delta1-delta1.raw)),
    Abis.delta2.ORDUTree_2 <- mean(abs(para.ORDUTree_2[(I+1):(3*I),4]*sign.ORDUTree_2.delta2-beta2.raw)),
    Abis.tau1.ORDUTree_2 <- mean(abs(para.ORDUTree_2[1:I,5]-tau1.raw)),
    ...
    Bias.delta1.ERSUTree <- mean(para.ERSUTree[1:I,3]*sign.ERSUTree.delta1-delta1.raw),
    Bias.beta2.ERSUTree <- mean(para.ERSUTree[(I+1):(3*I),9]*sign.ERSUTree.beta2-beta2.raw),
    Bias.tau1.ERSUTree <- mean(para.ERSUTree[1:I,5]-tau1.raw),
    ...
    Bias.delta1.ORDUTree_2 <- mean(para.ORDUTree_2[1:I,3]*sign.ORDUTree_2.delta1-delta1.raw),
    Bias.delta2.ORDUTree_2 <- mean(para.ORDUTree_2[(I+1):(3*I),4]*sign.ORDUTree_2.delta2-beta2.raw),
    Bias.tau1.ORDUTree_2 <- mean(para.ORDUTree_2[1:I,5]-tau1.raw),
    ...
    RMSE.delta1.ERSUTree <- RMSE(unname(para.ERSUTree[1:I,3])*sign.ERSUTree.delta1,delta1.raw),
    RMSE.beta2.ERSUTree <- RMSE(unname(para.ERSUTree[(I+1):(3*I),9])*sign.ERSUTree.beta2,beta2.raw),
    RMSE.tau1.ERSUTree <- RMSE(unname(para.ERSUTree[1:I,5]),tau1.raw),
    ...
    RMSE.delta1.ORDUTree_2 <- RMSE(unname(para.ORDUTree_2[1:I,3])*sign.ORDUTree_2.delta1,delta1.raw),
    RMSE.delta2.ORDUTree_2 <- RMSE(unname(para.ORDUTree_2[(I+1):(3*I),4])*sign.ORDUTree_2.delta2,beta2.raw),
    RMSE.tau1.ORDUTree_2 <- RMSE(unname(para.ORDUTree_2[1:I,5]),tau1.raw),
    ...
    # 95% confidence interval (theta and eta) include true value
    theta_95CItrue.ERS <- mean(ifelse(data.table::between(theta.raw,theta_95CI.ERS[,1],theta_95CI.ERS[,2]),1,0)),
    eta_95CItrue.ERS <- mean(ifelse(data.table::between(eta.raw,eta_95CI.ERS[,1],eta_95CI.ERS[,2]),1,0)),
    theta_95CItrue.ORD_1 <- mean(ifelse(data.table::between(theta.raw,theta_95CI.ORD_1[,1],theta_95CI.ORD_1[,2]),1,0)),
    theta1_95CItrue.ORD_2 <- mean(ifelse(data.table::between(theta.raw,theta1_95CI.ORD_2[,1],theta1_95CI.ORD_2[,2]),1,0)),
    theta2_95CItrue.ORD_2 <- mean(ifelse(data.table::between(eta.raw,theta2_95CI.ORD_2[,1],theta2_95CI.ORD_2[,2]),1,0)),
    theta_95CItrue.ERSUTree <- mean(ifelse(data.table::between(theta.raw,theta_95CI.ERSUTree[,1],theta_95CI.ERSUTree[,2]),1,0)),
    eta_95CItrue.ERSUTree <- mean(ifelse(data.table::between(eta.raw,eta_95CI.ERSUTree[,1],eta_95CI.ERSUTree[,2]),1,0)),
    theta_95CItrue.ORDUTree_1 <- mean(ifelse(data.table::between(theta.raw,theta_95CI.ORDUTree_1[,1],theta_95CI.ORDUTree_1[,2]),1,0)),
    theta1_95CItrue.ORDUTree_2 <- mean(ifelse(data.table::between(theta.raw,theta1_95CI.ORDUTree_2[,1],theta1_95CI.ORDUTree_2[,2]),1,0)),
    theta2_95CItrue.ORDUTree_2 <- mean(ifelse(data.table::between(eta.raw,theta2_95CI.ORDUTree_2[,1],theta2_95CI.ORDUTree_2[,2]),1,0)),
    ...
    theta.relia_Jin.ERS <- 1-(var(theta.esti.ERS-theta.raw)/var(theta.raw)),
    eta.relia_Jin.ERS <-1-(var(eta.esti.ERS-eta.raw)/var(eta.raw)),
    theta.relia_Jin.ORD_1 <- 1-(var(theta.esti.ORD_1-theta.raw)/var(theta.raw)),
    theta1.relia_Jin.ORD_2 <- 1-(var(theta1.esti.ORD_2-theta.raw)/var(theta.raw)),
    theta2.relia_Jin.ORD_2 <-1-(var(theta2.esti.ORD_2-eta.raw)/var(eta.raw)),
    theta.relia_Jin.ERSUTree <- 1-(var(theta.esti.ERSUTree-theta.raw)/var(theta.raw)),
    eta.relia_Jin.ERSUTree <-1-(var(eta.esti.ERSUTree-eta.raw)/var(eta.raw)),
    theta.relia_Jin.ORDUTree_1 <- 1-(var(theta.esti.ORDUTree_1-theta.raw)/var(theta.raw)),
    theta1.relia_Jin.ORDUTree_2 <- 1-(var(theta1.esti.ORDUTree_2-theta.raw)/var(theta.raw)),
    theta2.relia_Jin.ORDUTree_2 <- 1-(var(theta2.esti.ORDUTree_2-eta.raw)/var(eta.raw)),
...
## 5. Run simulation 
Sim.ERSUTreedata <- runSimulation(design = Design,
                                 replications =100,
                                 parallel = T ,
...
colnames(Sim.ERSUTreedata) <- c(colnames(Sim.ERSUTreedata)[1:4],
                                ...
                                "Abis.alpha1.ERSUTree","Abis.alpha2.ERSUTree","Abis.delta1.ERSUTree","Abis.beta2.ERSUTree","Abis.tau1.ERSUTree","Abis.cor.ERSUTree",
                                "Abis.alpha1.ORDUTree_1","Abis.alpha2.ORDUTree_1","Abis.delta1.ORDUTree_1","Abis.delta2.ORDUTree_1","Abis.tau1.ORDUTree_1",
                                "Abis.alpha1.ORDUTree_2","Abis.alpha2.ORDUTree_2","Abis.delta1.ORDUTree_2","Abis.delta2.ORDUTree_2","Abis.tau1.ORDUTree_2","Abis.cor.ORDUTree_2",
                                ...
                                "Bias.alpha1.ERSUTree","Bias.alpha2.ERSUTree","Bias.delta1.ERSUTree","Bias.beta2.ERSUTree","Bias.tau1.ERSUTree","Bias.cor.ERSUTree",
                                "Bias.alpha1.ORDUTree_1","Bias.alpha2.ORDUTree_1","Bias.delta1.ORDUTree_1","Bias.delta2.ORDUTree_1","Bias.tau1.ORDUTree_1",
                                "Bias.alpha1.ORDUTree_2","Bias.alpha2.ORDUTree_2","Bias.delta1.ORDUTree_2","Bias.delta2.ORDUTree_2","Bias.tau1.ORDUTree_2","Bias.cor.ORDUTree_2",
                                ...
                                "RMSE.alpha1.ERSUTree","RMSE.alpha2.ERSUTree","RMSE.delta1.ERSUTree","RMSE.beta2.ERSUTree","RMSE.tau1.ERSUTree","RMSE.cor.ERSUTree",
                                "RMSE.alpha1.ORDUTree_1","RMSE.alpha2.ORDUTree_1","RMSE.delta1.ORDUTree_1","RMSE.delta2.ORDUTree_1","RMSE.tau1.ORDUTree_1",
                                "RMSE.alpha1.ORDUTree_2","RMSE.alpha2.ORDUTree_2","RMSE.delta1.ORDUTree_2","RMSE.delta2.ORDUTree_2","RMSE.tau1.ORDUTree_2","RMSE.cor.ORDUTree_2",
...
write.table(Sim.ERSUTreedata,"sim_4models_ERSUTreedata_final_rep100_20240802.csv",row.names = F,sep = ",")
saveRDS(Sim.ERSUTreedata,"Sim_4models_ERSUTreedata_final_rep100_20240802.rds")

detail_ERSUTreedata <- SimResults(Sim.ERSUTreedata)
saveRDS(detail_ERSUTreedata,"Sim_4models_detail.ERSUTreedata_final_rep100_20240802.rds")
save.image("Sim_4models_detail.ERSUTreedata_final_rep100_20240802.RData")
