...
setwd("C:/Unfolding Tree Model/ORDUTree_2")
  ...
  data.gen <- function(alpha1,delta1,tau1,alpha2,delta2,tau2,
    ...
    for (n in 1:N){
      ...
      for (i in 1:I) {
        ...
        sum1.0 <- 1+exp(alpha1[i]*3*(theta1[n]-delta1[i]))
        sum1.1 <- exp(alpha1[i]*(1*(theta1[n]-delta1[i])+sum(tau1[i,1:2])))+exp(alpha1[i]*(2*(theta1[n]-delta1[i])+sum(tau1[i,1:2])))
        
        sum2.0 <- 1+exp(alpha2[i]*3*(theta2[n]-delta2[i]))
        sum2.1 <- exp(alpha2[i]*(1*(theta2[n]-delta2[i])+sum(tau2[i,1:2])))+exp(alpha2[i]*(2*(theta2[n]-delta2[i])+sum(tau2[i,1:2])))
        
        sum3.0 <- 1+exp(alpha2[I+i]*3*(theta2[n]-delta2[I+i]))
        sum3.1 <- exp(alpha2[I+i]*(1*(theta2[n]-delta2[I+i])+sum(tau2[I+i,1:2])))+exp(alpha2[I+i]*(2*(theta2[n]-delta2[I+i])+sum(tau2[I+i,1:2])))
        ...
        ## Probability for the node 3 (ideal point model)
    ...
    dat <- list(data.4,data.2,data.6, alpha1.raw,delta1.raw,tau1.raw,
                alpha2.raw,delta2.raw,tau2.raw,theta1.raw,theta2.raw)
    dat
  }
...
Analyse<-function(condition,dat,fixed_objects=NULL){
  ...
  sign.ERSUTree.beta2 <- sign(cor(para.ERSUTree[(I+1):(3*I),9],delta2.raw))
  ...
  sign.ORDUTree_2.delta2 <- sign(cor(para.ORDUTree_2[(I+1):(3*I),4], delta2.raw))
  ...
  ret<-c(
    ...
    Abis.beta2.ERSUTree <- mean(abs(para.ERSUTree[(I+1):(3*I),9]*sign.ERSUTree.beta2-delta2.raw)),
    ...
    Abis.delta2.ORDUTree_2 <- mean(abs(para.ORDUTree_2[(I+1):(3*I),4]*sign.ORDUTree_2.delta2-delta2.raw)),
    Abis.tau1.ORDUTree_2 <- mean(abs(para.ORDUTree_2[1:I,5]-tau1.raw)),
    Abis.tau2.ORDUTree_2 <- mean(abs(para.ORDUTree_2[(I+1):(3*I),5]-tau2.raw)),
    ...
    Bias.beta2.ERSUTree <- mean(para.ERSUTree[(I+1):(3*I),9]*sign.ERSUTree.beta2-delta2.raw),
    ...
    Bias.delta2.ORDUTree_2 <- mean(para.ORDUTree_2[(I+1):(3*I),4]*sign.ORDUTree_2.delta2-delta2.raw),
    Bias.tau1.ORDUTree_2 <- mean(para.ORDUTree_2[1:I,5]-tau1.raw),
    Bias.tau2.ORDUTree_2 <- mean(para.ORDUTree_2[(I+1):(3*I),5]-tau2.raw),
    ...
    RMSE.beta2.ERSUTree <- RMSE(unname(para.ERSUTree[(I+1):(3*I),9])*sign.ERSUTree.beta2,delta2.raw),
    ...
    RMSE.delta2.ORDUTree_2 <- RMSE(unname(para.ORDUTree_2[(I+1):(3*I),4])*sign.ORDUTree_2.delta2,delta2.raw),
    RMSE.tau1.ORDUTree_2 <- RMSE(unname(para.ORDUTree_2[1:I,5]),tau1.raw),
    RMSE.tau2.ORDUTree_2 <- RMSE(unname(para.ORDUTree_2[(I+1):(3*I),5]),tau2.raw),
...
## 5. Run simulation 
Sim.ORDUTree_2data <- runSimulation(design = Design,
                                 replications =100,
                                 parallel = T ,
...
colnames(Sim.ORDUTree_2data) <- c(colnames(Sim.ORDUTree_2data)[1:4],
                                  ...
                                  "Abis.alpha1.ORDUTree_2","Abis.alpha2.ORDUTree_2","Abis.delta1.ORDUTree_2","Abis.delta2.ORDUTree_2","Abis.tau1.ORDUTree_2","Abis.tau2.ORDUTree_2","Abis.cor.ORDUTree_2",
                                  ...
                                  "Bias.alpha1.ORDUTree_2","Bias.alpha2.ORDUTree_2","Bias.delta1.ORDUTree_2","Bias.delta2.ORDUTree_2","Bias.tau1.ORDUTree_2","Bias.tau2.ORDUTree_2","Bias.cor.ORDUTree_2",
                                  ...
                                  "RMSE.alpha1.ORDUTree_2","RMSE.alpha2.ORDUTree_2","RMSE.delta1.ORDUTree_2","RMSE.delta2.ORDUTree_2","RMSE.tau1.ORDUTree_2","RMSE.tau2.ORDUTree_2","RMSE.cor.ORDUTree_2",
...
write.table(Sim.ORDUTree_2data,"sim_ORDUTree_2data_final rep100_20240802.csv",row.names = F,sep = ",")
saveRDS(Sim.ORDUTree_2data,"Sim_ORDUTree_2data_final rep100_20240802.rds")
save.image("Sim_ORDUTree_2data_final rep100_20240802.RData")
